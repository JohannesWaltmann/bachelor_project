...
Since the esp32 Chipset does not naturally support the arduinos own \textbf{\textit{tone()}}-function it needs to be implemented separately.
For this purpose the LED PWM functions can be used.

There you first have to call the methods \textit{ledcSetup()} and \textit{ledcAttachPin()} during the controllers general setup-function.
With these methods the base duty cycle and base frequency are set as well as the pin to which the speaker component will be attached. 

When changing the duty cycle at constant frequency it is possible to shift the pitch/the tone colour of the sound using the \textit{ledcWrite-function}.
When changing the frequency at constant duty cycle it is possible to generate different notes.
To change the frequency the function \textit{ledcWriteTone()} has to be used instead of \textit{ledcWrite()}.
\begin{lstlisting}[language={c++}, caption={Function used to generate Audio output}, label={lst:toneFunct}]
...
\end{lstlisting}
je kleiner delayAmount berechnet ist desto höher wird die letztendliche Note

loopTime berechnet die Länge des Tons/der Note

im Loop wird dann für Tonlänge mit jedem Looptick der digitalWrite alternierend auf high und low gesetzt und mit dem Delay die Tonhöhe reguliert
-> Am ende wird ein kleiner delay eingebaut um einzelne Noten von einander abzutrennen

Methodenparameter geben an über welchen Pin die Methode ausgeführt werden soll sowie benötigte Frequenz und Länge um einen bestimmten Ton abzuspielen
